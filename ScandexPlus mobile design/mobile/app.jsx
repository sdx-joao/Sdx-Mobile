...
function HomeScreen({ cfg, user, onGoOrders, onGoInventory, onGoRecords, onOpenWO, onOpenItem }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', background: T.bg, padding: '16px 16px 24px' }}>
        ...
        <button onClick={onGoRecords} style={{ width: '100%', textAlign: 'left', cursor: 'pointer', background: `linear-gradient(135deg, ${cfg.accent}, ${T.primaryDark})`, border: 'none', borderRadius: 16, padding: 16, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 14, fontFamily: T.font, boxShadow: `0 10px 24px -10px ${cfg.accent}99` }}>
          <div style={{ width: 46, height: 46, borderRadius: 13, background: 'rgba(255,255,255,.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <Icon name="file-text" size={23} color="#fff" />
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: '#fff', letterSpacing: -0.2 }}>Prontuários</div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,.8)', marginTop: 2 }}>Buscar paciente e documentos</div>
          </div>
          <Icon name="chevron-right" size={20} color="rgba(255,255,255,.85)" />
        </button>
        <div style={{ display: 'flex', gap: 10, marginBottom: 22 }}>
...
function TabBar({ active, onChange, accent }) {
  const tabs = [
    ...
    { key: 'records', label: 'Prontuários', icon: 'file-text' },
...
function App() {
  ...
  const go = (key) => { setOverlay(null); setRecDoc(null); setTab(key); if (scrollRef.current) scrollRef.current.scrollTop = 0; };
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '30px 0', background: 'radial-gradient(circle at 50% 0%, #1c2438, #0b0f1a)' }}>
      <PhoneFrame statusDark={authed && !overlay && tab === 'home' ? true : (overlay ? true : (authed ? false : true))}>
        ...
        <div ref={scrollRef} style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
          {tab === 'home' && <HomeScreen cfg={cfg} user={USER} onGoOrders={() => go('orders')} onGoInventory={() => go('inventory')} onGoRecords={() => go('records')} onOpenWO={openWO} onOpenItem={openItem} />}
          {tab === 'records' && <RecordsScreen cfg={cfg} onOpenDoc={(s, p) => setRecDoc({ surgery: s, patient: p })} />}
        ...
        {recDoc && <DocViewer surgery={recDoc.surgery} patient={recDoc.patient} onBack={() => setRecDoc(null)} />}
