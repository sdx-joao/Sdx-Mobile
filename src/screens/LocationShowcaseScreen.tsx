...
function ItemPhoto({ item, height = 82 }: { item: PublicLocationItem; height?: number }) {
  ...
  if (uri) return <Image source={{ uri }} resizeMode="cover" style={{ width: '100%', height }} />;
  return (
    ...
    <View style={{ flex: 1, backgroundColor: '#F4F7FB' }}>
      <ScrollView contentContainerStyle={{ paddingBottom: 36 }} showsVerticalScrollIndicator={false}>
        <View style={{ backgroundColor: T.primary, paddingTop: insets.top + 10, paddingHorizontal: 18, paddingBottom: 82, overflow: 'hidden' }}>
          ...
          <Text style={{ marginTop: 10, color: '#fff', fontSize: 31, fontWeight: '900', lineHeight: 34 }}>
            {data?.location.roomName || data?.location.sectorName || 'Carregando local'}
          </Text>
          {!!data && (
            <Text style={{ marginTop: 9, color: '#DBEAFE', fontSize: 13.5, fontWeight: '600' }}>
        ...
        <View style={{ marginTop: -55, marginHorizontal: 15, borderRadius: 24, backgroundColor: '#fff', flexDirection: 'row', paddingVertical: 19, shadowColor: '#0F172A', shadowOpacity: .12, shadowRadius: 22, elevation: 7 }}>
          {[
            ...
            <View key={String(label)} style={{ flex: 1, alignItems: 'center', borderLeftWidth: index ? 1 : 0, borderLeftColor: '#EEF2F7' }}>
              ...
              <Text style={{ marginTop: 3, color: '#94A3B8', fontSize: 9, fontWeight: '800', textTransform: 'uppercase' }}>{label}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={{ paddingHorizontal: 15, paddingTop: 28 }}>
          ...
          {!!error && (
            <View style={{ marginTop: 18, borderRadius: 20, padding: 22, backgroundColor: '#fff' }}>
              <Text style={{ color: '#64748B', textAlign: 'center', fontWeight: '700' }}>{error}</Text>
            </View>
          )}
          <View style={{ gap: 11, marginTop: 17 }}>
            {data?.items.map(item => (
              <Pressable key={item.id} onPress={() => setSelected(item)}
                style={{ flexDirection: 'row', alignItems: 'center', gap: 13, padding: 11, borderRadius: 21, backgroundColor: '#fff', borderWidth: 1, borderColor: '#EEF2F7' }}>
                <View style={{ width: 82, height: 82, borderRadius: 16, overflow: 'hidden' }}><ItemPhoto item={item} /></View>
                <View style={{ flex: 1, minWidth: 0 }}>
                  <Text numberOfLines={1} style={{ color: '#0F172A', fontSize: 14.5, fontWeight: '900' }}>{item.name}</Text>
                  <Text numberOfLines={1} style={{ marginTop: 4, color: '#94A3B8', fontSize: 11.5, fontWeight: '600' }}>
                  ...
                  <View style={{ marginTop: 10, flexDirection: 'row', gap: 6 }}>
                    ...
                    <Text style={{ borderRadius: 99, backgroundColor: item.condition === 'Funcionando' ? '#ECFDF5' : '#FFF1F2', paddingHorizontal: 8, paddingVertical: 4, color: item.condition === 'Funcionando' ? '#047857' : '#BE123C', fontSize: 9.5, fontWeight: '900' }}>{item.condition}</Text>
                  </View>
                </View>
            ...
            <Pressable onPress={() => undefined} style={{ maxHeight: '94%', borderTopLeftRadius: 32, borderTopRightRadius: 32, backgroundColor: '#fff', padding: 15, paddingBottom: Math.max(24, insets.bottom + 12) }}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={{ paddingHorizontal: 8, paddingTop: 21 }}>
                  <Text style={{ color: T.primary, fontSize: 10.5, fontWeight: '900', letterSpacing: 1.5 }}>{selected.category || 'ITEM INVENTARIADO'}</Text>
                  <Text style={{ marginTop: 8, color: '#0F172A', fontSize: 27, fontWeight: '900', lineHeight: 31 }}>{selected.name}</Text>
                  <Text style={{ marginTop: 7, color: '#64748B', fontSize: 13, fontWeight: '600' }}>{[selected.brand, selected.model].filter(Boolean).join(' · ')}</Text>
                  <View style={{ flexDirection: 'row', gap: 10, marginTop: 21 }}>
                    {[['PATRIMÔNIO', selected.assetTag || 'Sem patrimônio'], ['SITUAÇÃO', selected.situation]].map(([label, value]) => (
                      <View key={label} style={{ flex: 1, borderRadius: 17, backgroundColor: '#F8FAFC', padding: 14 }}>
                        ...
                        <Text style={{ marginTop: 6, color: '#0F172A', fontSize: 14, fontWeight: '900' }}>{value}</Text>
                      </View>
                    ))}
                  </View>
                  <View style={{ marginTop: 10, borderRadius: 17, padding: 15, flexDirection: 'row', justifyContent: 'space-between', backgroundColor: selected.condition === 'Funcionando' ? '#ECFDF5' : '#FFF1F2' }}>
                    ...
                    <Text style={{ color: selected.condition === 'Funcionando' ? '#047857' : '#BE123C', fontWeight: '900' }}>{selected.condition}</Text>
                  </View>
                </View>
              </ScrollView>
            </Pressable>
